          ...
          <div v-else>
            <div class="card-header">
              <div class="row">
                ...
                <router-link :to="{name: 'AddBrand'}" class="btn btn-primary col-lg-2 col-md-2 col-sm-2 me-1">
                  {{ $t('add') }} +
                </router-link>

              </div>
            </div>
            <div class="card-body px-0 pt-0 pb-2" v-if="listOfBrands.length > 0">
              <DataTable ref="dt"
                         :value="listOfBrands"
                         dataKey="id"
                         removableSort
...
import {fetchAssets, fetchBranches, fetchBrands, fetchUsers} from "@/services/apiService/getApiService";
...
onMounted(async () => {
  ...
  try {
    ...
    const resultBrands = await fetchBrands();
    ...
    // updateLoadingPercentage()
    if (resultBrands.status === 200) {
      listOfBrands.value = resultBrands.data.results;
      // updateLoadingPercentage()
