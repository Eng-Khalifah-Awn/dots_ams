        ...
        <div class="card">
          ...
          <div v-else>
            <div class="card-header">
              <div class="row">
                <h6 class="col-lg-5 col-md-5 col-sm-5">{{ $t('brand_table') }}</h6>
                ...
                <router-link :to="{name: 'AddDepartment'}" class="btn btn-primary col-lg-2 col-md-2 col-sm-2 me-1">
                  {{ $t('add') }} +
                </router-link>

              </div>
            </div>
            <div class="card-body px-0 pt-0 pb-2" v-if="listOfDepartments.length > 0">
              <DataTable ref="dt"
...
const listOfDepartments = ref([]);
const appStore = useAppStore();


const loading = ref(true)
const editProduct = (department) => {
  router.push({ name: 'UpdateDepartment', params: { departmentId: department.id } })
}
const rowsPerPage = ref(10)
const filters = ref({
  global: { value: null, matchMode: FilterMatchMode.CONTAINS },
  name_department: { value: null, matchMode: FilterMatchMode.STARTS_WITH },
})
...
onMounted(async () => {
  ...
  try {
    ...
    const resultDepartments = await fetchDepartments();
    ...
    // updateLoadingPercentage()
    if (resultDepartments.status === 200) {
      listOfDepartments.value = resultDepartments.data.results;
      // updateLoadingPercentage()
