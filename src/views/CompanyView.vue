          ...
          <div v-else>
            <div class="card-header">
              <div class="row">
                ...
                <router-link :to="{name: 'AddCompany'}" class="btn btn-primary col-lg-2 col-md-2 col-sm-2 me-1">
                  {{ $t('add') }} +
                </router-link>

              </div>
            </div>
            <div class="card-body px-0 pt-0 pb-2" v-if="listOfCompanies.length > 0">
              <DataTable ref="dt"
...
import {fetchAssets, fetchBranches, fetchBrands, fetchCompanies, fetchUsers} from "@/services/apiService/getApiService";
...
const editProduct = (company) => {
  router.push({ name: 'UpdateCompany', params: { companyId: company.id } })
}
...
onMounted(async () => {
  ...
  try {
    ...
    const resultCompanies = await fetchCompanies();
    ...
    if (resultCompanies.status === 200) {
      listOfCompanies.value = resultCompanies.data.results;
      // updateLoadingPercentage()
